// Centralized project data for reuse (listing and intelligent preloading)
export const projectsData = [
  {
    id: "ciphermaniac",
    title: "Ciphermaniac",
    path: "https://ciphermaniac.com/",
    github: "https://github.com/rosematcha/ciphermaniac",
    thumbnail: "/img/projects/ciphermaniac-thumb.webp",
    thumbnailWidth: 900,
    thumbnailHeight: 1300,
    description:
      "To show my local league players what cards are frequently used in the Pokemon card game, I created this tool tracking the usage of cards in major tournaments. Now serves thousands of daily users.",
    tech: ["react", "node.js", "tailwindcss"],
    tags: ["data viz", "pokemon"],
    featured: true,
  },
  {
    id: "matchatab",
    title: "MatchaTab",
    path: "https://addons.mozilla.org/en-US/firefox/addon/matchatab/",
    thumbnail: "/img/projects/matchatab-thumb.webp",
    thumbnailWidth: 330,
    thumbnailHeight: 183,
    description:
      "Inspired by Tabliss, a deeply customizable and modular new tab page to meet my needs. Built for Firefox browsers (my preference.)",
    tech: ["javascript", "css"],
    tags: ["browser extension", "weekend project"],
    buttonText: "Install on Firefox",
    featured: true,
  },
  {
    id: "saysi",
    title: "SAY Sí",
    path: "https://saysi.org",
    thumbnail: "/img/projects/saysi.webp",
    thumbnailWidth: 900,
    thumbnailHeight: 666,
    description:
      'I redesigned and upgraded SAY Sí\'s WordPress site, updating from an outdated PHP client and building a <a href="https://github.com/rosematcha/buttercup" target="_blank" rel="noopener noreferrer">bespoke WordPress plugin</a> to increase user flexibility. The new extensible system allows non-technical editors to better highlight the work the org does.',
    tech: ["wordpress", "php", "javascript"],
    tags: ["client work", "nonprofits", "custom plugin", "arts", "youth"],
    featured: true,
  },
  {
    id: "upo",
    title: "United Professional Organizers",
    path: "https://upounion.org",
    thumbnail: "/img/projects/upo-thumb.webp",
    thumbnailWidth: 654,
    thumbnailHeight: 404,
    description:
      "I overhauled UPO's website from Google Sites to WordPress for greater editor accessibility and a modernized appearance. I built a member portal in integration with Patreon for union members to access confidential info.",
    tech: ["wordpress", "patreon", "php"],
    tags: ["client work", "member portal", "migration", "labor"],
  },
  {
    id: "stonewallaction",
    title: "Stonewall Action",
    path: "https://stonewallaction.org/",
    thumbnail: "/img/projects/stonewallaction-thumb.webp",
    thumbnailWidth: 654,
    thumbnailHeight: 404,
    description:
      "As part of a consulting contract with Stonewall Action, a queer activism nonprofit out of Austin, I redesigned their website. I valued legibility and accessibility to serve our inclusive mission.",
    tech: ["wordpress", "php"],
    tags: ["client work", "nonprofits", "accessibility", "lgbtq"],
  },
  {
    id: "balatro3ds",
    title: "Balatro3DS",
    path: "https://github.com/rosematcha/Balatro3DS/releases",
    github: "https://github.com/rosematcha/Balatro3DS",
    thumbnail: "/img/projects/balatro3ds-thumb.webp",
    thumbnailWidth: 840,
    thumbnailHeight: 560,
    description:
      "A port of one of my favorite video games to the Nintendo 3DS utilizing a port of its engine to the console. Forked from a project that started the work, but completed referencing the game's original code.",
    tech: ["lua", "lövepotion"],
    tags: ["homebrew", "3ds", "game port"],
    buttonText: "download a release",
  },
  {
    id: "sanjapan",
    title: "De-Guidebooked San Japan Guide",
    path: "https://sanjapan.rosematcha.com/",
    github: "https://github.com/rosematcha/deguidebooked-san-japan",
    thumbnail: "/img/projects/sanjapan-thumb.webp",
    thumbnailWidth: 900,
    thumbnailHeight: 599,
    description:
      "I decompiled the San Japan Android app and extracted the convention's schedule to make it freely available without an account. Additionally, I used OCR to make the app's maps smarter, and added inter-linking between listings, guests, and locations.",
    tech: ["javascript", "node.js", "swift"],
    tags: ["static site", "reverse engineering", "ocr", "anime convention"],
  },
  {
    id: "filmscraper",
    title: "FilmScraper",
    path: "https://filmscraper.rosematcha.com/",
    github: "https://github.com/rosematcha/filmscraper",
    thumbnail: "/img/projects/filmscraper-thumb.webp",
    thumbnailWidth: 900,
    thumbnailHeight: 599,
    description:
      "Wanting a more user-friendly view of movies showing in town, I created a minimal frontend utilizing data from Fandango.",
    tech: ["typescript", "playwright", "netlify"],
    tags: ["static site", "movies", "scraper"],
  },
  {
    id: "gesso",
    title: "Gesso",
    path: "https://usegesso.com/",
    github: "https://github.com/usegesso",
    thumbnail: "/img/projects/gesso.webp",
    thumbnailWidth: 1200,
    thumbnailHeight: 741,
    description:
      "The artists I work with every day struggled to find an easy-to-use and autonomy-respecting portfolio builder. I built Gesso to empower artists to build their own portfolios in a way that respects their ownership and personality, for free.",
    tech: ["astro"],
    tags: ["arts", "portfolio", "no-code", "free"],
  },
  {
    id: "brahdb",
    title: "BrahDB",
    path: "/brahdb",
    thumbnail: "/img/projects/brahdb-thumb.webp",
    thumbnailWidth: 333,
    thumbnailHeight: 184,
    description:
      "Processed data visualization from YouTube's finest fast food reviewer, ReportOfTheWeek, with data scraped, collected, and transformed from subtitles using Python.",
    tech: ["vite", "react", "typescript"],
    tags: ["data viz", "weekend project"],
  },
  {
    id: "bexarvoting",
    title: "Bexar County Voting Data",
    path: "/bexarvoting",
    thumbnail: "/img/projects/bexarvoting-thumb.webp",
    thumbnailWidth: 327,
    thumbnailHeight: 184,
    description:
      "Dissatisfied with the County's lack of transparency, I created a site to visualize historical and present Bexar County voting data.",
    tech: ["javascript", "chart.js", "tailwindcss"],
    tags: ["data viz", "civic tech"],
  },
  {
    id: "friendfinder",
    title: "Favorite Film Friend Finder",
    path: "/friendfinder",
    thumbnail: "/img/projects/friendfinder-thumb.webp",
    thumbnailWidth: 327,
    thumbnailHeight: 184,
    description:
      '<a href="https://x.com/rachellapides/status/1868875739732623414" target="_blank" rel="noopener noreferrer">Inspired by a Tweet,</a> a simple tool for finding Letterboxd users with your same top four favorite films.',
    tech: ["javascript", "html", "css"],
    tags: ["static site", "letterboxd", "weekend project"],
  },
  {
    id: "moviemap",
    title: "Movie Map",
    path: "/moviemap",
    thumbnail: "/img/projects/moviemap-thumb.jpg",
    thumbnailWidth: 327,
    thumbnailHeight: 184,
    description: "Visualizing the connections between people's four favorite movies on Letterboxd.",
    tech: ["d3.js", "python"],
    tags: ["data viz", "letterboxd", "weekend project"],
  },
];
